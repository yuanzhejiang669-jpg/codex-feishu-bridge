// background.js - Codex Browser Control Bridge
try {
  importScripts('bridge-token.local.js');
} catch {
  // Public repo fallback; real installs should provide bridge-token.local.js.
}

const BRIDGE_PORT = Number(globalThis.CODEX_BROWSER_BRIDGE_PORT || 18795);
const BRIDGE_TOKEN = String(globalThis.CODEX_BROWSER_BRIDGE_TOKEN || '<local-extension-token>');
const ALLOW_UNSAFE_CSP_BYPASS = false;
const ALLOW_EXTENSION_MANAGEMENT = false;
const ALLOW_CONTENT_SETTINGS = false;

chrome.runtime.onInstalled.addListener(() => {
  console.log('Codex Browser Control Bridge installed');
  if (!chrome.declarativeNetRequest) return;
  const addRules = ALLOW_UNSAFE_CSP_BYPASS ? [{
      id: 9999, priority: 1,
...
async function handleExtMessage(msg, sender) {
  ...
  if (msg.cmd === 'management') {
    if (!ALLOW_EXTENSION_MANAGEMENT || !chrome.management) {
      return { ok: false, error: 'extension management commands are disabled' };
    }
    try {
      if (msg.method === 'list') {
    ...
    chrome.alarms.create('codex-browser-bridge-self-reload', { when: Date.now() + 200 });
        return { ok: true };
      }
  ...
  if (msg.cmd === 'contentSettings') {
    if (!ALLOW_CONTENT_SETTINGS || !chrome.contentSettings) {
      return { ok: false, error: 'contentSettings commands are disabled' };
    }
    try {
...
function buildExecScript(code, errorHandler) {
  return `(async () => {
    function smartProcessResult(result) {
      ...
      try { return JSON.parse(JSON.stringify(result, function(key, value) { if (typeof value === 'object' && value !== null) { if (value.nodeType === 1) return value.outerHTML; if (value === window || value === document) return '[Object]'; try { if (value.window === value && value.document) return '[Window]'; } catch(_){} } return value; })); } catch (e) { return '[鏃犳硶搴忓垪鍖? ' + e.message + ']'; }
    }
    try {
...
let ws = null;
const WS_URL = `ws://127.0.0.1:${BRIDGE_PORT}/?token=${encodeURIComponent(BRIDGE_TOKEN)}`;

function scheduleProbe() {
  // Use chrome.alarms to survive MV3 service worker suspension
  chrome.alarms.create('codex-browser-bridge-ws-probe', { delayInMinutes: 0.083 }); // ~5s
}

function scheduleKeepalive() {
  // Keep SW alive while WS is connected (~25s, under 30s SW timeout)
  chrome.alarms.create('codex-browser-bridge-ws-keepalive', { delayInMinutes: 0.4 }); // ~24s
}
...
async function isServerAlive() {
  try {
    ...
    await fetch(`http://127.0.0.1:${BRIDGE_PORT}`, { signal: ctrl.signal });
    return true; // Got HTTP response 鈫?port is listening
  } catch (e) {
    return false; // Network error (connection refused) or timeout 鈫?server not alive
  }
}

chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'codex-browser-bridge-self-reload') {
    chrome.runtime.reload();
    return;
  }
  if (alarm.name === 'codex-browser-bridge-ws-keepalive') {
  ...
  if (alarm.name === 'codex-browser-bridge-ws-probe') {
    if (ws && ws.readyState <= 1) return; // Already connected/connecting
    if (await isServerAlive()) {
      console.log('[CodexBridge-WS] Server detected, connecting...');
      connectWS();
    } else {
...
async function handleWsExec(data) {
  ...
  console.log('[CodexBridge-WS] Exec request', data.id, 'on tab', tabId);
  ...
  try {
    ...
    try {
      ...
      if (res === null || res === undefined) {
        console.log('[CodexBridge-WS] executeScript returned null/undefined, treating as CSP issue');
        res = { ok: false, error: { name: 'Error', message: 'executeScript returned null (possible CSP or context issue)', stack: '' }, csp: true };
      }
    } catch (e) {
      console.log('[CodexBridge-WS] scripting.executeScript failed:', e.message);
    ...
    if (res && !res.ok && res.csp) {
      console.log('[CodexBridge-WS] CDP fallback for tab', tabId);
...
function connectWS() {
  ...
  ws = null;
  console.log('[CodexBridge-WS] Connecting to', WS_URL);
  try {
    ws = new WebSocket(WS_URL);
  } catch (e) {
    console.error('[CodexBridge-WS] Constructor error:', e);
    ws = null;
    scheduleProbe();
    return;
  }
  ws.onopen = async () => {
    console.log('[CodexBridge-WS] Connected!');
    ...
    console.log('[CodexBridge-WS] Sent ext_ready with', tabs.length, 'tabs');
  };
    ...
    try {
      ...
      if (data.id && data.code) {
        ...
        if (typeof code === 'object' && code !== null && code.cmd) {
          // Custom protocol message 鈫?route to handleExtMessage
          ...
          // Object without cmd 鈫?legacy extension message
      ...
      console.error('[CodexBridge-WS] message parse error', e);
    }
  };
  ws.onclose = () => {
    console.log('[CodexBridge-WS] Disconnected');
    ws = null;
    scheduleProbe();
  };
  ws.onerror = (e) => {
    console.error('[CodexBridge-WS] Error:', e);
...
function wakeAndSendTabsUpdate() {
  ensureBridgeConnection();
  sendTabsUpdate().catch(e => console.warn('[CodexBridge-WS] tabs_update failed:', e.message));
}

chrome.tabs.onUpdated.addListener((_, changeInfo) => {
  if (changeInfo.status === 'complete') wakeAndSendTabsUpdate();
});
chrome.tabs.onRemoved.addListener(() => wakeAndSendTabsUpdate());
chrome.tabs.onCreated.addListener(() => wakeAndSendTabsUpdate());
chrome.tabs.onActivated.addListener(() => wakeAndSendTabsUpdate());
chrome.windows.onFocusChanged.addListener(() => wakeAndSendTabsUpdate());
