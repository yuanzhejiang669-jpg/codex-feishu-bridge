...
- GitHub Actions run `31859040642` passed the clean Windows test, package verification, asset upload, and stable publish jobs. Public Release `v0.8.18` contains exactly five Windows assets and is neither draft nor prerelease.
- The freshly downloaded public installer is `176955409` bytes with SHA-256 `6357d3a27c47be751186e8e18a029911211f2d837689e00ac74d5b25bc00df93`, matching both the published checksum and GitHub asset digest.
...
- The first clean rebuild from pushed commit `b3f5449` passed dependency installation, protocol-proxy smoke, engine staging and Linux tool preparation, then stopped before producing a `.deb` because the physical Ubuntu host lacked `ar`. Installed `binutils` as the explicit native-build prerequisite and documented it so a future release host fails during preparation rather than being mistaken for an application defect.

## 2026-08-30 - Ubuntu in-app update 0.8.19-linux.2

- Added a Linux-specific GitHub Release adapter while retaining the existing shared updater state machine and install transaction. It scans only Releases containing an x64 `.deb` plus `checksums-linux.txt`, compares stable and `-linux.N` versions explicitly, downloads through bounded requests, and never treats Windows or macOS assets as Linux updates.
- The installer accepts only an installed `/opt/Codex Feishu Bridge/` DEB client with `apt-get`, PolicyKit, Bash, and SHA-256 available. It verifies the GitHub asset digest, the release checksum manifest, and the downloaded file, then launches a packaged user-session helper. Installation still requires normal PolicyKit authorization; no updater-specific passwordless sudo rule is created.
- Existing active-run protection remains authoritative. Installation re-reads every managed Bot, blocks while any `active-runs.json` has a non-empty `runs` map, writes the recovery marker, and stops only online idle Bots. Installer launch failure rolls the transaction back. Authorization cancellation or `apt-get` failure relaunches the client so the recovery marker can restore the previous Bot set.
- Linux revision tags now use a separate prerelease publication job. `v*-linux.*` skips Windows/macOS builds and cannot update `main` or replace the stable Release; stable tags retain the existing three-platform release path.
- Version advanced from `0.8.19-linux.1` to `0.8.19-linux.2`. Windows verification passed 208/211 tests with three expected platform skips. Physical Ubuntu verification passed 205/214 tests with nine expected cross-platform skips. Both runs had zero failures. The Linux native build, protocol-proxy smoke, package extraction, dependency checks, bundled x64 tool checks, packaged engine verification, and packaged update-helper Bash syntax check all passed.
- The verified physical-device artifact is `/home/yzj666/Codex/releases/codex-feishu-bridge-linux-b3f5449/apps/desktop/out/Codex-Feishu-Bridge-0.8.19-linux.2-linux-amd64.deb`, 195,249,008 bytes, SHA-256 `b49deb09716c2fffbac165ae1ba1f540dc1e022f28be2a5926184ed1926c8652`. A direct unpacked Electron GUI smoke was not accepted as release evidence because an unpacked tree lacks the DEB-installed setuid sandbox and the SSH shell lacked X authorization; the package verifier and already-running installed DEB are the relevant checks.

Adversarial review for the three most likely failures after three months:

1. The updater could choose an unrelated stable asset or treat a Linux preview as older forever. The candidate selector now derives the version from the exact `*-linux-amd64.deb` asset, requires the checksum asset in the same Release, handles `-linux.N` revisions separately, and has stable/preview/draft/foreign-asset regression tests. Linux-only tags are isolated from the stable publication job.
2. A partial download, stale 200 MB cache, or modified package could reach `apt-get`. Downloads use a temporary file and atomic rename, three SHA-256 values must agree (GitHub asset digest, manifest, local file), invalid packages are deleted, bounded timeouts prevent indefinite hangs, and old version cache directories are pruned without touching unrelated files.
3. An update could interrupt a late-starting task or leave Bots offline after a canceled administrator prompt. The install plan is re-evaluated immediately before stopping; the existing stop path rejects a Bot that becomes active during the race; recovery state is written before any stop; installer-launch errors roll back; and the detached helper relaunches the client after success, cancellation, checksum refusal, or package-manager failure. Active-run blocking, rollback, helper launch, and recovery behavior are covered by tests.

- Publication and installed upgrade completed from reviewed commit `e1ddd1499360e606bb35ea4a507bd63eef5d0867`. GitHub Actions run `33301041833` passed the Linux build and isolated prerelease publication while the Windows and macOS jobs were skipped. Public prerelease `v0.8.19-linux.2` contains `Codex-Feishu-Bridge-0.8.19-linux.2-linux-amd64.deb` (GitHub SHA-256 `9add761a1c605a4c02e26282c1aac85c1619afd9b3f5e165a80861b8d7dc812f`) and `checksums-linux.txt`; the stable `v0.8.19` Release and `main` remained unchanged.
- The physical Ubuntu device downloaded the public assets, passed `sha256sum -c`, and installed package version `0.8.19~linux.2`. Installation waited until every real `runs` map was empty. The replacement desktop process started as PID `347927`; the five managed Bots then received replacement PIDs `351273`, `351683`, `352095`, `352586`, and `353259`. They remained stable for 30 seconds with zero active runs, matching PID files, and five connected `im.message.receive_v1` consumers. Two orphaned helper processes from the previous desktop instance were terminated normally after the device became idle.
