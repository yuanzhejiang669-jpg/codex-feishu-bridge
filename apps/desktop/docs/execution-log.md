...
- Publication and installed upgrade are deliberately pending until the reviewed branch is committed, pushed, and the exact `0.8.19-linux.2` prerelease assets are verified on GitHub. Before installation, the physical device must again show empty `runs` objects for every `active-runs.json`.
