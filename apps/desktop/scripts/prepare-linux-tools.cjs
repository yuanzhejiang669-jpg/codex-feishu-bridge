...
const architectures = requested.length ? [...new Set(requested)] : ["x64"];
...
for (const architecture of architectures) {
  ...
  const targetRoot = path.join(generatedRoot, `linux-${architecture}`);
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), `cfb-linux-tools-${architecture}-`));
  try {
    fs.rmSync(targetRoot, { recursive: true, force: true });
    fs.mkdirSync(targetRoot, { recursive: true });

    const larkArchiveName = `lark-cli-${larkVersion}-linux-${larkArch}.tar.gz`;
    ...
    const nodeArchiveName = `node-v${nodeVersion}-linux-${architecture}.tar.xz`;
    ...
    execFileSync("tar", ["-xJf", nodeArchive, "-C", temporary], { stdio: "inherit" });
    copyExecutable(
      path.join(temporary, `node-v${nodeVersion}-linux-${architecture}`, "bin", "node"),
    ...
    process.stdout.write(`Prepared Linux ${architecture} tools\n`);
  } finally {
